...
import "./Menu.css";
//Utils
import { menuLink } from "../../utils/routesName";
//React Router Dom
...
const Menu = () => {
  ...
  return (
    <div onKeyDown={(e) => arrowsActions(e)} className="menu">
      ...
      <Link className="menu_link" to={menuLink.edit}>
        <p>{text.load} (admin)</p>
        <ArrowBackIosIcon
          className={`${selected === 1 ? "active" : "no-active"}`}
        />
      </Link>
      <Link className="menu_link" to={menuLink.options}>
        <p>{text.title_options}</p>
        <ArrowBackIosIcon
          className={`${selected === 2 ? "active" : "no-active"}`}
        />
      </Link>
      <div className="menu__back">
        <Link className="menu_link" to={menuLink.back}>
          <p>{text.back}</p>
          <ArrowBackIosIcon
